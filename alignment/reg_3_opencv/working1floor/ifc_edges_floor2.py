...
def extract_ifc_plan_edges(
    ...
    floor2_min, floor2_max = ranges[1]

    # --- FLOOR 2 FILTER ---
    floor2_segments: List[Segment] = []
    for x1, y1, x2, y2, z in raw_segments:
        if floor2_min <= z < floor2_max:
            floor2_segments.append((x1, y1, x2, y2))

    # Fallbacks (in case avg_z is weird for your model)
    if not floor2_segments:
        # fallback 1: just remove site/ground
        floor2_segments = [(x1, y1, x2, y2) for (x1, y1, x2, y2, z) in raw_segments if z > z_threshold]

    if not floor2_segments:
        # fallback 2: give up and keep all
        floor2_segments = [(x1, y1, x2, y2) for (x1, y1, x2, y2, z) in raw_segments]

    # --- RE-CENTER AND BBOX ---
    pts = np.array(
        [[s[0], s[1]] for s in floor2_segments] + [[s[2], s[3]] for s in floor2_segments],
        dtype=float
    )
    minx, miny = np.min(pts, axis=0)
    maxx, maxy = np.max(pts, axis=0)

    shifted_segments = [(x1 - minx, y1 - miny, x2 - minx, y2 - miny) for (x1, y1, x2, y2) in floor2_segments]
...
if __name__ == "__main__":
    ...
    path = sys.argv[1]
    print(f"Processing Floor 2 only: {path}")

    segs, meta = extract_ifc_plan_edges_floor2_only(path)

    # Debug plot
    plt.figure(figsize=(10, 10))
    for x1, y1, x2, y2 in segs:
        plt.plot([x1, x2], [y1, y2], linewidth=0.5)
    plt.title(f"IFC Floor 2 Only: {meta.bbox[2]:.2f}x{meta.bbox[3]:.2f} units")
    plt.axis("equal")
    plt.grid(True, linestyle="--", alpha=0.6)

    output_path = "/home/chidepnek/RoboAI/BIM/BIM2/backend/src/reg_3_opencv/working1floor/ifc_floor2_only.png"
